...
class ProposalServiceTest {
 ...
 @Test void ownerAcceptsProposal(){Proposal p=new Proposal(UUID.randomUUID(),land,buyer,BigDecimal.ONE,"Offer",Instant.now());Proposal other=new Proposal(UUID.randomUUID(),land,new User(UUID.randomUUID(),"Other","other@test.com","x",Instant.now()),BigDecimal.TEN,"Offer",Instant.now());when(users.require("owner@test.com")).thenReturn(owner);when(repository.findById(p.getId())).thenReturn(Optional.of(p));when(repository.findByLandIdAndStatus(land.getId(),ProposalStatus.PENDING)).thenReturn(List.of(p,other));assertThat(service.update(p.getId(),ProposalStatus.ACCEPTED,"owner@test.com").getStatus()).isEqualTo(ProposalStatus.ACCEPTED);assertThat(other.getStatus()).isEqualTo(ProposalStatus.REJECTED);}
